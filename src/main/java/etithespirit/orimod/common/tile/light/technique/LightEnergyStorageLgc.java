package etithespirit.orimod.common.tile.light.technique;


import etithespirit.orimod.energy.IPersistentLightEnergyStorage;
...
public class LightEnergyStorageLgc implements IPersistentLightEnergyStorage {
	...
	public Runnable markDirty; // For telling attached block entities that they have changed.
	...
	public LightEnergyStorageLgc(@Nullable Runnable markDirty, float capacity) {
		this(markDirty, capacity, capacity, capacity, 0);
	}
	
	public LightEnergyStorageLgc(@Nullable Runnable markDirty, float capacity, float maxTransfer) {
		this(markDirty, capacity, maxTransfer, maxTransfer, 0);
	}
	
	public LightEnergyStorageLgc(@Nullable Runnable markDirty, float capacity, float maxReceive, float maxExtract) {
	 ...
	 */
	public LightEnergyStorageLgc(@Nullable Runnable markDirty, float capacity, float maxReceive, float maxExtract, float energy) {
	...
	@Override
	public void loadEnergyInfo(CompoundTag tag) {
		energy = tag.getFloat(ENERGY_KEY);
	}
}
